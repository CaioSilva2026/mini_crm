...
def list_leads():
    ...
    for i, lead in enumerate(leads):
        print(f"{i:02d} | {lead["nome"]:<10} | {lead["email"]:<10} | {lead["company"]}")
...
def search_leads():
    ...
    # emvia a query para o control realizar a busca no leads.json


def export_leads():
    path_csv = control.export_leads()

    if path_csv is None:
        print("Não foi possível exportar as leads")
    else:
